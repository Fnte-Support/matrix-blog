...
AI 模型常在長文中意外夾入簡體字。發文前請自我檢查。常見錯誤對照（**左錯 → 右對**）：
...
**最常出錯的地方是「標題」**（AI 會在標題用簡體但內文全繁體）。發文前請特別檢查 `title` 欄位。

### 2. 咖啡術語必須用台灣精品咖啡圈慣用詞

| ❌ 避免 | ✅ 正確 | 說明 |
|---|---|---|
| 手衝、手沖 | **手沖** | SCA 官譯、本站統一用字 |
| 拿提 | **拿鐵** | Latte |
...
| 奶泡壶 | **奶泡壺** | 繁體 |
| 压粉 | **壓粉**、**填壓** | 繁體 |
| 萃取率、萃取濃度 | 兩者都可，但不要寫成「提取率」 | |
| 单品、单品咖啡 | **單品咖啡** / **Single Origin** | 繁體 |
...
12. **不要混入簡體字**。發前掃一遍 title + description + body_html，確認零簡體（參考上方繁體用字規範）
13. **不要用「手衝」「拿提」「滤杯」「义式」**等非台灣慣用詞。用「手沖」「拿鐵」「濾杯」「Espresso / 濃縮咖啡」
...
- [ ] **標題與內文全數為繁體中文（台灣）**，沒有簡體字混入（特別檢查 title 的「黃 / 發 / 體 / 風 / 學 / 為 / 從」等常見易混字）
- [ ] 咖啡術語用台灣圈慣用詞：手沖（非手衝）、拿鐵（非拿提）、磨豆機（非研磨机）、濾杯（非滤杯）等
