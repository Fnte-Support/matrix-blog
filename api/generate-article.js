...
function buildSystemPrompt() {
  return [
    ...
    "至少用到 4 種不同元件。",
    "",
    "【延伸閱讀：不要自己產生】你無法得知站內實際有哪些文章，所以不要編造延伸閱讀連結（會變死連結），由人工後製補上。",
    "【資料來源】若文中用到具體數據/研究，文末用 dc-sources 列出，且【盡量每一筆都附上連結】：",
    '<div class="dc-sources"><p>資料來源</p><ul><li><a href="https://sca.coffee/" target="_blank" rel="noopener noreferrer">Specialty Coffee Association</a></li></ul></div>',
    "連結一律指向該來源的【官方首頁網域】（你有高度把握存在的，例如 sca.coffee、en.wikipedia.org、worldcoffeeresearch.org、ncausa.org、britannica.com），不要編造深層文章網址。預設就應該帶官網首頁連結；只有連官網首頁都沒把握時，那一筆才退而只寫來源名稱、不附連結。絕不可編造或亂猜網址。",
    "",
...
function buildUserPrompt({ title, category, topic }) {
  ...
  return [
    ...
    `  "body_markdown": "完整內文，markdown + 上述視覺元件 HTML 混用。順序：dc-quick 30 秒重點框 → 引言（不加標題）→ 5–8 個 ## H2（每 1–2 段穿插視覺元件）→ FAQ（<details><summary>Q：問題</summary>答案</details>）→（有引用才放）dc-sources 資料來源。不要自己產生延伸閱讀連結。正文純文字至少 3000 字（目標 3000–5000），紮實有深度、不灌水。表格只用於多項目比較（≥3欄×3列），單一資訊用 dc-info-card。",`,
    `  "images": [`,
