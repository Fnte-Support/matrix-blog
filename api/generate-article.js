...
function buildSystemPrompt() {
  return [
    ...
    "",
    "【文末必備兩段】",
    '- 延伸閱讀（3 條相關咖啡主題，用 dc-related；連結用 # 佔位，由編輯改成站內文章）：<div class="dc-related"><p>📖 延伸閱讀</p><ul><li><a href="#">相關主題一</a></li><li><a href="#">相關主題二</a></li><li><a href="#">相關主題三</a></li></ul></div>',
    '- 資料來源（若文中用到具體數據/研究才放，誠實列出，不可編造連結，用 dc-sources）：<div class="dc-sources"><p>資料來源</p><ul><li>來源名稱或連結</li></ul></div>',
    "",
...
function buildUserPrompt({ title, category, topic }) {
  ...
  return [
    ...
    `  "body_markdown": "完整內文，markdown + 上述視覺元件 HTML 混用。順序：dc-quick 30 秒重點框 → 引言（不加標題）→ 5–8 個 ## H2（每 1–2 段穿插視覺元件）→ FAQ（<details><summary>Q：問題</summary>答案</details>）→ 文末 dc-related 延伸閱讀 →（有引用才放）dc-sources 資料來源。正文純文字至少 3000 字（目標 3000–5000），紮實有深度、不灌水。表格只用於多項目比較（≥3欄×3列），單一資訊用 dc-info-card。",`,
    `  "images": [`,
